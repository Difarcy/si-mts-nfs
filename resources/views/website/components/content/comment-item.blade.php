    ...
    <div class="flex items-start gap-3 sm:gap-4">
        ...
        <div class="flex-1 font-lato min-w-0">
            ...
            <div class="flex items-center gap-4 mt-2 pt-0">
                <button type="button" 
                    class="flex items-center gap-1.5 text-sm font-bold {{ $comment->total_likes > 0 ? 'text-green-700' : 'text-slate-900' }} hover:text-green-700 transition-colors group"
                    data-action="like-comment" 
                    data-id="{{ $comment->id }}">
                    
                    <svg xmlns="http://www.w3.org/2000/svg" 
                        class="h-5 w-5 {{ $comment->total_likes > 0 ? 'hidden' : 'text-gray-400 group-hover:text-green-700' }}" 
                        fill="none" viewBox="0 0 24 24" stroke="currentColor" data-icon="outline">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                    </svg>

                    <svg xmlns="http://www.w3.org/2000/svg" 
                        class="h-5 w-5 {{ $comment->total_likes > 0 ? 'text-green-700 fill-current' : 'hidden' }}" 
