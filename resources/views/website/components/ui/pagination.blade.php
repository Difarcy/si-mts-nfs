...
<div {{ $attributes->merge(['class' => 'flex items-center justify-between gap-2']) }}>
    ...
    <div class="flex gap-1 text-black items-center">
        ...
        @else
            <span class="px-2 py-1 sm:px-3 sm:py-1 border border-black rounded text-[10px] sm:text-xs text-black/40 flex items-center gap-1 cursor-not-allowed bg-gray-50">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                <span class="hidden sm:inline">Previous</span>
            </span>
        @endif
        ...
        @if($isLengthAware && count($pageLinks) > 0)
            <div class="hidden sm:flex items-center gap-1">
                @foreach($pageLinks as $page)
                    @if($page === '...')
                        <span class="px-2 py-1 sm:px-3 sm:py-1 border border-black rounded text-[10px] sm:text-xs text-black/40 cursor-default bg-gray-50">…</span>
