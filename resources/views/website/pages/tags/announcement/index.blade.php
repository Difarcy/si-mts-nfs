...
@section('title', 'Tag Pengumuman')

@section('content')
    <div class="space-y-6 pt-4 sm:pt-6">
        <x-website.components.layout.breadcrumb :items="[['label' => 'TAGS'], ['label' => 'PENGUMUMAN'], ['label' => strtoupper($decodedTag)]]" />
        ...
        <div class="w-full space-y-4" data-website-search-scope>
            ...
            @php
                $announcementsList = $posts ?? collect();
            @endphp
            ...
            @if($announcementsList->isEmpty())
                <div class="flex flex-col items-center justify-center min-h-[400px] text-center w-full">
                    <p class="text-[11px] sm:text-base font-semibold text-slate-900 tracking-wider">
                        Tidak ada pengumuman untuk tag ini
                    </p>
                </div>
            @else
                <div class="space-y-4 sm:space-y-5">
                    @foreach($announcementsList as $item)
                        ...
                        <article
                            ...
                            data-website-search-item data-search="{{ e(($item->judul ?? '') . ' ' . $excerpt . ' ' . ($item->tags ?? '')) }}">
