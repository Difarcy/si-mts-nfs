...
@section('title', 'Tag Prestasi Siswa')

@section('content')
    <div class="pt-4 sm:pt-6 space-y-6">
        <x-website.components.layout.breadcrumb :items="[['label' => 'TAGS'], ['label' => 'PRESTASI SISWA'], ['label' => strtoupper($decodedTag)]]" />
        <x-website.components.layout.page-title title="Tag: {{ $decodedTag }}" />

        @php
            $achievements = $posts ?? collect();
        @endphp
            ...
            @if($achievements->isEmpty())
                <div class="flex flex-col items-center justify-center min-h-[400px] text-center w-full">
                    <p class="text-[11px] sm:text-base font-semibold text-slate-900 tracking-wider">
                        Tidak ada prestasi untuk tag ini
                    </p>
                </div>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6" data-website-search-scope>
                    @foreach($achievements as $achievement)
                        <div class="group bg-white rounded-xl shadow-md hover:shadow-xl overflow-hidden transition-all duration-300 flex flex-col h-full border border-gray-100"
                            ...
                            data-search="{{ e(($achievement->nama_lomba ?? '') . ' ' . ($achievement->nama_siswa ?? '') . ' ' . strip_tags($achievement->deskripsi ?? '') . ' ' . ($achievement->tags ?? '')) }}">
                            <a href="{{ route('web.achievement.detail', $achievement) }}" class="relative aspect-[4/3] overflow-hidden bg-gray-50 block">
                                @if($achievement->foto_siswa)
                                    <img src="{{ asset('storage/' . $achievement->foto_siswa) }}" alt="{{ $achievement->nama_lomba }}"
                ...
                @if(method_exists($achievements, 'links'))
                    <div class="mt-10">
                        <x-website.components.ui.pagination :items="$achievements" />
                    </div>
                @endif
            @endif
        </div>
