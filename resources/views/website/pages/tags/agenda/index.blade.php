...
@section('title', 'Tag Agenda')

@section('content')
    <div class="space-y-6 pt-4 sm:pt-6">
        <x-website.components.layout.breadcrumb :items="[['label' => 'TAGS'], ['label' => 'AGENDA'], ['label' => strtoupper($decodedTag)]]" />
            ...
            @if($agendasList->isEmpty())
                <div class="flex flex-col items-center justify-center min-h-[400px] text-center w-full">
                    <p class="text-[11px] sm:text-base font-semibold text-slate-900 tracking-wider">
                        Tidak ada agenda untuk tag ini
                    </p>
                </div>
            @else
                <div class="space-y-4">
                    @foreach($agendasList as $item)
                        <article
                            ...
                            data-search="{{ e(($item->judul ?? '') . ' ' . ($item->lokasi ?? '') . ' ' . strip_tags($item->deskripsi ?? '') . ' ' . ($item->tags ?? '')) }}">
