...
@section('title', ($item->judul ?? 'Detail Prestasi') . ' | MTs Nurul Falaah Soreang')
...
@section('content')
    <div class="space-y-6 pt-4 sm:pt-6">
        ...
        <x-website.components.layout.breadcrumb :items="[
            ['label' => 'PROFIL'],
            ['label' => 'PRESTASI SISWA', 'url' => route('web.achievement')],
            ['label' => $item->nama_lomba ?? $item->judul]
        ]" />

        <x-website.components.layout.page-title title="Detail Prestasi Siswa" />

        @php
            $publishedAt = $item->tanggal_publikasi;
            $dateObj = $publishedAt ?? $item->tanggal ?? now();
            $date = \Carbon\Carbon::parse($dateObj)->translatedFormat('d F Y');
            $time = $publishedAt ? \Carbon\Carbon::parse($publishedAt)->format('H:i') : null;
            $shareUrl = url()->current();
            $shareText = 'Prestasi Siswa: ' . ($item->nama_lomba ?? $item->judul) . ' - ' . ($item->nama_siswa ?? '');
            ...
            @if($tagsList->count() > 0)
                <div class="flex flex-wrap gap-2">
                    @foreach($tagsList as $tag)
                        <a href="{{ route('web.tags.achievement', ['tag' => $tag]) }}"
        ...
        <!-- Featured Image (Foto Siswa) -->
        @if($item->foto_siswa)
            <div class="w-full overflow-hidden border border-gray-100 bg-gray-50 rounded-lg">
                <img src="{{ asset('storage/' . $item->foto_siswa) }}" alt="{{ $item->judul }}"
                    class="w-full h-auto max-h-[520px] object-contain mx-auto" loading="lazy">
            </div>
        @endif

        <!-- Informasi Prestasi Box -->
        <div class="pt-6 border-t border-gray-100">
            <h3 class="text-base sm:text-lg font-bold text-black mb-4 font-roboto-slab">Informasi Prestasi</h3>
            @php
                $tingkat = $item->tingkat;
                $jenis = $item->jenis;
            @endphp
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-10 font-lato">
                <div>
                    <dt class="text-xs sm:text-sm text-slate-900 mb-1">Nama Siswa</dt>
                    <dd class="text-sm sm:text-base font-semibold text-black">{{ $item->nama_siswa ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs sm:text-sm text-slate-900 mb-1">Kelas</dt>
                    <dd class="text-sm sm:text-base font-semibold text-black">{{ $item->kelas ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs sm:text-sm text-slate-900 mb-1">Kategori</dt>
                    <dd class="text-sm sm:text-base font-semibold text-black">
                        @if($tingkat && $jenis)
                            {{ $tingkat }} <span class="text-black">•</span> {{ $jenis }}
                        @elseif($tingkat)
                            {{ $tingkat }}
                        @elseif($jenis)
                            {{ $jenis }}
                ...
                <div>
                    <dt class="text-xs sm:text-sm text-slate-900 mb-1">Tanggal Lomba</dt>
                    <dd class="text-sm sm:text-base font-semibold text-black">
                        {{ $item->tanggal ? \Carbon\Carbon::parse($item->tanggal)->translatedFormat('d F Y') : '-' }}
                    </dd>
                </div>
                <div>
                    <dt class="text-xs sm:text-sm text-slate-900 mb-1">Peringkat</dt>
                    <dd class="text-sm sm:text-base font-semibold text-black">{{ $item->peringkat ?? '-' }}</dd>
                </div>
                <div class="sm:col-span-2">
                    <dt class="text-xs sm:text-sm text-slate-900 mb-1">Penyelenggara</dt>
                    <dd class="text-sm sm:text-base font-semibold text-black">{{ $item->penyelenggara ?? '-' }}</dd>
                </div>
            </dl>
        </div>
        ...
        <div class="prose prose-base sm:prose-base max-w-none text-black leading-relaxed font-lato text-justify">
            ...
            @if($item->sertifikat)
                <div class="mt-8 not-prose">
                    <h3 class="text-base sm:text-lg font-bold text-black mb-4 font-roboto-slab text-center">Sertifikat / Piagam</h3>
                    <div class="max-w-2xl mx-auto" data-component="upload-image">
                        <div class="upload-preview-container cursor-zoom-in">
                            <div class="w-full overflow-hidden rounded-lg aspect-[297/210]">
                                <img src="{{ asset('storage/' . $item->sertifikat) }}" alt="Sertifikat"
                                    class="w-full h-full object-contain" loading="lazy">
                            </div>
                        </div>
                        <div class="mt-2 text-center">
                            <p class="text-xs text-slate-900 italic">Klik gambar untuk zoom</p>
                        </div>
                    </div>
                </div>
            @endif
        ...
        <x-website.components.content.comments contentType="achievement" :contentId="$item->id" :comments="$comments ?? collect()" />
    </div>
@endsection
