...
@section('title', ($post->judul ?? 'Berita') . ' | MTs Nurul Falaah Soreang')
...
@section('content')
    <div class="space-y-6 pt-4 sm:pt-6">
        ...
        <x-website.components.layout.breadcrumb :items="[
            ['label' => 'INFORMASI'],
            ['label' => 'BERITA', 'url' => route('web.news')],
            ['label' => $post->judul ?? 'Detail Berita']
        ]" />

        <x-website.components.layout.page-title title="Detail Berita" />
        ...
        @php
            ...
            $shareText = (string) ($post->judul ?? 'Berita');
            ...
            @if($tagsList->count() > 0)
                <div class="flex flex-wrap gap-2">
                    @foreach($tagsList as $tag)
                        <a href="{{ route('web.tags.news', ['tag' => $tag]) }}"
                ...
                @foreach($images as $path)
                    <div class="overflow-hidden border border-gray-100 bg-gray-50 aspect-video rounded-lg">
                        <img src="{{ str_starts_with($path, 'http') ? $path : asset('storage/' . $path) }}"
                            alt="{{ $post->judul ?? 'Berita' }}" class="w-full h-full object-cover" loading="lazy">
                    </div>
                @endforeach
            </div>
        @endif
        ...
        <x-website.components.content.comments contentType="news" :contentId="$post->id" :comments="$comments ?? collect()" />
    </div>
@endsection
