...
@section('title', ($post->judul ?? 'Agenda') . ' | MTs Nurul Falaah Soreang')

@section('content')
    <div class="space-y-6 pt-4 sm:pt-6">
        <!-- Breadcrumb -->
        <x-website.components.layout.breadcrumb :items="[['label' => 'INFORMASI'], ['label' => 'AGENDA', 'url' => route('web.agenda')], ['label' => $post->judul ?? 'Detail Agenda']]" />

        <!-- Header Section -->
        <x-website.components.layout.page-title title="Detail Agenda" />
        ...
        @php
            ...
            $shareText = (string) ($post->judul ?? 'Agenda');
        ...
        <div class="space-y-2">
            ...
            @if($tagsList->count() > 0)
                <div class="flex flex-wrap gap-2">
                    @foreach($tagsList as $tag)
                        <a href="{{ route('web.tags.agenda', ['tag' => $tag]) }}"
            ...
            <h1 class="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-black font-roboto-slab leading-tight">
                {{ $post->judul ?? 'Judul Kegiatan Sekolah' }}
            </h1>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <!-- Tanggal Card -->
            <div
                class="p-4 bg-green-50/50 border border-green-100 rounded-xl text-center flex flex-col items-center group hover:bg-green-50 transition-colors">
                <div
                    class="w-10 h-10 bg-green-700 text-white rounded-xl flex items-center justify-center mb-3 shadow-md transform group-hover:scale-110 transition-transform">
                ...
                <span class="text-[10px] font-bold text-slate-900 uppercase tracking-widest mb-1 font-lato">Tanggal</span>
                <p class="text-xs sm:text-sm font-bold text-black font-roboto-slab leading-tight">
                    {{ isset($post->tanggal_mulai) ? \Carbon\Carbon::parse($post->tanggal_mulai)->translatedFormat('d F Y') : '-' }}
                    @if(isset($post->tanggal_selesai) && $post->tanggal_selesai->format('Y-m-d') != $post->tanggal_mulai->format('Y-m-d'))
                        <span class="block text-[10px] text-slate-900 font-medium py-0.5 font-lato">s/d</span>
                        {{ \Carbon\Carbon::parse($post->tanggal_selesai)->translatedFormat('d F Y') }}
                    @endif
                </p>
            </div>
            <!-- Waktu Card -->
            <div
                ...
                <span class="text-[10px] font-bold text-slate-900 uppercase tracking-widest mb-1 font-lato">Waktu</span>
                <p class="text-xs sm:text-sm font-bold text-black font-roboto-slab uppercase">
                    @php
                        $start = isset($post->waktu_mulai) ? \Carbon\Carbon::parse($post->waktu_mulai)->format('H.i') : '08.00';
                        $endTime = isset($post->waktu_selesai) ? \Carbon\Carbon::parse($post->waktu_selesai)->format('H:i') : null;
            ...
            <div
                ...
                <span class="text-[10px] font-bold text-slate-900 uppercase tracking-widest mb-1 font-lato">Lokasi</span>
                <p class="text-xs sm:text-sm font-bold text-black font-roboto-slab leading-tight">
                    {{ $post->lokasi ?? 'Di Sekolah' }}
                </p>
            </div>
        </div>
        ...
        @if($images->isNotEmpty())
            <div class="grid {{ $images->count() === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2' }} gap-3">
                @foreach($images as $path)
                    <div class="overflow-hidden border border-gray-100 bg-gray-50 aspect-video rounded-xl">
                        <img src="{{ str_starts_with($path, 'http') ? $path : asset('storage/' . $path) }}"
                            alt="{{ $post->judul ?? 'Agenda' }}" class="w-full h-full object-cover" loading="lazy">
                    </div>
                @endforeach
            </div>
        @endif
        ...
        <div class="prose prose-base sm:prose-base max-w-none text-black leading-relaxed text-justify font-lato">
            {!! $post->deskripsi ?? '<p>Deskripsi lengkap agenda sedang tidak tersedia.</p>' !!}
        </div>
            ...
            <div class="pt-6 border-t border-gray-100">
                <div
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-sm font-bold text-black font-roboto-slab">Lampiran Informasi Agenda</p>
                            <p class="text-[11px] text-slate-900 font-lato mt-0.5">Format file:
        ...
        <x-website.components.content.comments contentType="agenda" :contentId="$post->id" :comments="$comments ?? collect()" />
    </div>
@endsection
