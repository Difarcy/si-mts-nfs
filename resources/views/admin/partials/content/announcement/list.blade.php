                ...
                <tr class="hover:bg-gray-50/50 transition-colors" data-admin-item="table" data-id="{{ $item->id }}"
                    ...
                    <td class="px-2 sm:px-4 py-2 sm:py-3 text-center text-black">
                        <div class="flex flex-col sm:flex-row items-center justify-center sm:gap-2 whitespace-nowrap">
                            ...
                            @if($item->tanggal_publikasi)
                                <div class="hidden sm:block w-px h-3 bg-black/40"></div>
                                <span
                    ...
                    <td class="px-2 sm:px-4 py-2 sm:py-3">
                        <div class="flex items-center justify-center gap-1 sm:gap-2">
                            ...
                            @else
                                <x-admin.form.button variant="edit" href="{{ route('admin.konten.pengumuman.edit', $item->id) }}"
                            ...
                            <x-admin.form.button variant="delete" class="!p-1.5 sm:!p-2" title="Hapus" data-announcement-delete
                                data-announcement-id="{{ $item->id }}" data-announcement-title="{{ $item->judul }}">
                                <x-slot:icon>
                                    <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
@else
    ...
    @if($hasSearch && $hasFilter)
        <div class="min-h-[300px] sm:min-h-[360px] flex flex-col items-center justify-center text-center">
            ...
            <p class="text-sm text-black mb-6 max-w-xs">Tidak ada pengumuman yang cocok dengan pencarian
    ...
    @elseif($hasSearch)
        <div class="min-h-[300px] sm:min-h-[360px] flex flex-col items-center justify-center text-center">
            ...
            <p class="text-sm text-black/60 mb-6 max-w-xs">Tidak ada pengumuman yang sesuai dengan pencarian
                "{{ request('search') }}".</p>
        </div>
    @elseif($hasFilter)
        <div class="min-h-[300px] sm:min-h-[360px] flex flex-col items-center justify-center text-center">
            ...
            <p class="text-sm font-semibold text-black mb-1">Data tidak ditemukan.</p>
            <p class="text-sm text-black/60 mb-6 max-w-xs">Tidak ada pengumuman yang cocok dengan filter yang dipilih.</p>
        </div>
    @else
        <div class="min-h-[300px] sm:min-h-[360px] flex flex-col items-center justify-center text-center">
            <svg class="w-24 h-24 text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
            ...
            <p class="text-sm text-black mb-6 max-w-xs">Mulai kelola pengumuman sekolah Anda dengan membuat pengumuman
                pertama.</p>
